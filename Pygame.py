...
from Problemclass import MultiplicationProblem
...
class Game:
    def __init__(self):
        ...
        self.choose_level_screen = False
    ...
    def handle_key_press(self, key):
        if self.start_screen:
            if key == pygame.K_RETURN:
                ...
                self.choose_level_screen = True
        elif self.choose_level_screen:
            if key == pygame.K_1 or key == pygame.K_KP1:
                ...
                self.problems = []
                self.lives = STARTING_LIVES
                self.current_mode = "multiplication"
                self.choose_level_screen = False
                self.game_over_screen = False
    ...
    def check_spawn(self):
        if self.tick % (120 // BUBBLE_FREQUENCY) == 0:
            if self.current_mode == "addition":
                self.problems.append(Problem())
            elif self.current_mode == "multiplication":
                self.problems.append(MultiplicationProblem())
    ...
    def save_score_to_db(self):
        con = sqlite3.connect('funny_bubbles.db')
        cur = con.cursor()
        cur.execute('INSERT INTO Scores (score) VALUES (?)', (self.score,))
        con.commit()
    ...
    def draw_choose_level_screen(self):
        self.screen.fill(ORANGE)
        choose_level_text_1 = self.big_font.render('CHOOSE LEVEL', True, WHITE)
        choose_level_text_2 = self.font.render('1 - Addition', True, WHITE)
        choose_level_text_3 = self.font.render('2 - Multiplication', True, WHITE)
        choose_level_rect_1 = choose_level_text_1.get_rect()
        choose_level_rect_2 = choose_level_text_2.get_rect()
        choose_level_rect_3 = choose_level_text_3.get_rect()
        choose_level_rect_1.centerx = WIDTH // 2
        choose_level_rect_1.y = HEIGHT // 2 - 50
        choose_level_rect_2.centerx = WIDTH // 2
        choose_level_rect_2.y = HEIGHT // 2 + 50
        choose_level_rect_3.centerx = WIDTH // 2
        choose_level_rect_3.y = HEIGHT // 2 + 100
        self.screen.blit(choose_level_text_1, choose_level_rect_1)
        self.screen.blit(choose_level_text_2, choose_level_rect_2)
        self.screen.blit(choose_level_text_3, choose_level_rect_3)
